...
console.info(`mcdata-to-json v${VERSION}`);
...
if (LOG_LEVELS.indexOf(LogLevel) > 2) console.log(`[${DOMAIN}] Finished parsing ARGV and .env`);
...
logger.verbose(`Logger initialized.`, { domain: DOMAIN });
...
logger.info(`Initializing internal path references`, { domain: DOMAIN });
...
if (PATHS.USERCACHE_FILE) {
  for (const p of JSON.parse(fs.readFileSync(PATHS.USERCACHE_FILE))) {
    if (p.uuid.indexOf("_old") === -1) {
      logger.debug(`Discovered cached player: ${p.name} ${p.uuid}`, { domain: DOMAIN });
      players[p.uuid] = p.name;
      pathref.ensureDirSync(path.join(PATHS.TEMP_PLAYERDATA_DIR, p.uuid));
    }
  }
}
...
for (const f of playerdatFiles) {
  if (f.indexOf(".dat") > -1 && f.indexOf("_old") === -1) {
    ...
    if (!players[uuid]) {
      ...
      pathref.ensureDirSync(path.join(PATHS.TEMP_PLAYERDATA_DIR, uuid));
    }
  }
}
