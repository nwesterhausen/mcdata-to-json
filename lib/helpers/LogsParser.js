...
function parseLogLine(basedate, logline) {
  if (logline.match(logregx.timestampRE)) {
    ...
    const sev = logline.match(logregx.severityRE)
      ? logline.match(logregx.severityRE)[1]
      : "ERROR";
    ...
    if (logline.match(logregx.playerjoinRE)) {
      return [
        timestamp,
        logconst.TYPE_LOGIN,
        logline.match(logregx.playerjoinRE)[1],
        sev,
      ];
    } else if (logline.match(logregx.playerleftRE)) {
      return [
        timestamp,
        logconst.TYPE_LOGOFF,
        logline.match(logregx.playerleftRE)[1],
        sev,
      ];
    } else if (logline.match(logregx.advancementRE)) {
      return [
        timestamp,
        logconst.TYPE_ADVANCEMENT,
        logline.match(logregx.advancementRE)[1],
        sev,
      ];
    } else if (logline.match(logregx.challengeRE)) {
      return [
        timestamp,
        logconst.TYPE_CHALLENGE,
        logline.match(logregx.challengeRE)[1],
        sev,
      ];
      ...
      return [
        timestamp,
        logconst.TYPE_GOAL,
        logline.match(logregx.goalRE)[1],
        sev,
      ];
      ...
      return [
        timestamp,
        logconst.ARROW_DEATH,
        logline.substr(logline.indexOf("]: ") + 3),
        sev,
      ];
    } else if (logline.match(logregx.cactusdeathRE)) {
      return [
        timestamp,
        logconst.CACTUS_DEATH,
        logline.substr(logline.indexOf("]: ") + 3),
        sev,
      ];
...
module.exports = {
  setConfig: function (config) {
    ...
    latestlogDate = fs
      .statSync(path.join(logfiledir, "latest.log"))
      .mtime.toISOString();
  ...
  prepareLogFiles: function () {
    ...
    log.debug(
      DOMAIN,
      `Preparing following log files: ${JSON.stringify(rawLogFiles)}`
    );
    ...
    for (let i = 0; i < rawLogFiles.length; i++) {
      ...
      if (rawLogFiles[i].endsWith(".gz")) {
        ...
        const compressedFile = fs.readFileSync(
          path.join(logfiledir, rawLogFiles[i])
        );
  ...
  combineLogFiles: function () {
    ...
    for (let i = 0; i < logfiles.length; i++) {
      ...
      const fileHeader =
        logfiles[i] === "latest.log"
          ? `[Filedate:${latestlogDate}.log]\n`
          : `[Filename:${logfiles[i]}]\n`;

      log.debug(DOMAIN, `Appending ${logfiles[i]} to temp.log.`);
      fs.appendFileSync(
        tmplogPath,
        fileHeader + fs.readFileSync(path.join(logfiledir, logfiles[i]))
      );
    }
  },
  parseLogFiles: function () {
    ...
    Promise.all(promises).then((files) => {
      ...
      const cleanedJSON = rawlogJSON.filter((obj) => {
        return (
          obj.type &&
          obj.type !== logconst.TYPE_KEEPENTITY &&
          obj.type !== logconst.TYPE_OVERLOADED
        );
      })

      fs.writeFileSync(
        path.join(workdir, "filtered_logs.json"),
      ...
      log.debug(
        DOMAIN,
        `Wrote 'cleaned' JSON file to ${path.join(
          workdir,
          "filtered_logs.json"
        )}`
      );
      ...
      fs.writeFileSync(
        path.join(workdir, "special_event_logs.json"),
        JSON.stringify(specialEventJSON)
      );
      log.info(
        DOMAIN,
        `${specialEventJSON.length} records determined worth saving.`
      );
      log.debug(
        DOMAIN,
        `Wrote 'important' JSON file to ${path.join(
          workdir,
          "special_event_logs.json"
        )}`
      );

      fs.writeFileSync(
        path.join(OUTPUT_DIR, "minecraft_logs.json"),
        JSON.stringify(specialEventJSON)
      );
    });
  },
