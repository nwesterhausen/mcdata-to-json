...
function resolvePaths() {
  ...
  logger.verbose(`Beginning path resolution from base paths.\n${JSON.stringify(BASE_PATHS)}`, { domain: DOMAIN });
  ...
  RESOLVED_PATHS.TEMP_PLAYERDATA_DIR = path.join(RESOLVED_PATHS.WORK_DIR, "playerdata");
  RESOLVED_PATHS.TEMP_LOG_JSON_DIR = path.join(RESOLVED_PATHS.WORK_DIR, "logs");
...
function validatePaths() {
  for (const path in RESOLVED_PATHS) {
    if (path !== "WORLD_DIRS") {
      logger.silly(`Validating ${path}, ${RESOLVED_PATHS[path]}`, { domain: DOMAIN });
      if (fs.existsSync(RESOLVED_PATHS[path])) {
        logger.debug(`${path} exists. (${RESOLVED_PATHS[path]})`, { domain: DOMAIN });
      } else {
        if (path.endsWith("_FILE")) {
          // Files we do not create, only check if they exist
          logger.warn(`${path} not found!`, { domain: DOMAIN });
          ...
          logger.debug(`Nonexistent path ${path} -> ${RESOLVED_PATHS[path]}`, { domain: DOMAIN });
          ensureDirSync(RESOLVED_PATHS[path]);
        } else {
  ...
  for (const worldPath in RESOLVED_PATHS.WORLD_DIRS) {
    if (fs.existsSync(RESOLVED_PATHS.WORLD_DIRS[worldPath])) {
      logger.debug(`${worldPath} exists. (${RESOLVED_PATHS.WORLD_DIRS[worldPath]})`, { domain: DOMAIN });
    } else {
      logger.warn(`Expected world path "${worldPath}" to exist but it doesn't.`, { domain: DOMAIN });
      logger.warn(`Any parsing of data in ${worldPath} will be skipped.`, { domain: DOMAIN });
      logger.debug(`Path for ${worldPath}: ${RESOLVED_PATHS.WORLD_DIRS[worldPath]}`, { domain: DOMAIN });
    }
  }
}
...
function setServerJarPath() {
  if (DETERMINATIONS.BUKKIT_LIKE && fs.existsSync(path.join(RESOLVED_PATHS.MC_DIR, "cache"))) {
    ...
    logger.debug(`Based on BUKKIT_LIKE confidence, checking for .jar cache.`, { domain: DOMAIN });
    ...
    logger.debug(`Found 'server.jar' for MCJAR_FILE`, { domain: DOMAIN });
...
/**
 * Creates a path if it doesn't exist. Otherwise does nothing.
 * @param {string} dirPath
 */
function ensureDirSync(dirPath) {
  if (!fs.existsSync(dirPath)) {
    fs.mkdirSync(dirPath, { recursive: true });
    logger.info(`Created directory at ${dirPath}`, { domain: DOMAIN });
  }
}
...
module.exports = {
  ...
  setBasePaths: function (mcdir, outdir, workdir) {
    logger.debug(`setBasePaths to mcdir:${mcdir}, outdir:${outdir}, workdir:${workdir}`, { domain: DOMAIN });
