...
function parseLogFiles() {
  prepareLogFiles();
  const tmplogPath = path.join(PATHS.TEMP_DIR, "all_logs.json");
  ...
  Promise.all(createJsonPromises)
    .then((files) => {
      ...
      fs.promises
        .writeFile(path.join(PATHS.TEMP_DIR, "filtered_logs.json"), JSON.stringify(cleanedJSON))
        .then(() => {
          logger.info(
            `${
          ...
          logger.debug(`Wrote 'cleaned' JSON file to ${path.join(PATHS.TEMP_DIR, "filtered_logs.json")}`, {
            domain: DOMAIN,
          });
        })
        .catch((err) => {
      ...
      fs.promises
        .writeFile(path.join(PATHS.TEMP_DIR, "special_event_logs.json"), JSON.stringify(specialEventJSON))
        .then(() => {
          logger.info(`${specialEventJSON.length} records determined worth saving.`, { domain: DOMAIN });
          logger.debug(`Wrote 'important' JSON file to ${path.join(PATHS.TEMP_DIR, "special_event_logs.json")}`, {
            domain: DOMAIN,
          });
