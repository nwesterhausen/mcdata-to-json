...
function jsonFromLogfilePromise(filepath) {
  return new Promise((resolve, reject) => {
    ...
    if (fs.existsSync(cachedLogJsonFilepath)) {
      ...
      if (fs.statSync(cachedLogJsonFilepath).mtime >= fs.statSync(filepath).mtime) {
        // Our cached version is at least as recent as the version we need to parse
        fs.promises
          .readFile(cachedLogJsonFilepath)
          .then(promiseParse)
          .then((logjson) => {
          ...
          .catch((err) =>
            logger.warn(`Unable to use cached JSON version of log, ${JSON.stringify(err)}`, { domain: DOMAIN })
          );
      }
    }
