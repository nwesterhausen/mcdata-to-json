...
function prepareLogFiles(config) {
  const rawLogFiles = fs.readdirSync(config.LOGS);
  ...
  for (let i = 0; i < rawLogFiles.length; i++) {
    ...
    if (rawLogFiles[i].endsWith(".gz")) {
      ...
      const compressedFile = fs.readFileSync(path.join(config.LOGS, rawLogFiles[i]));
      const unzippedFile = zlib.unzipSync(compressedFile);

      log.debug(DOMAIN, "Unzipping file into log dir.");
      fs.writeFileSync(path.join(config.LOGS, tmpLogFile), unzippedFile);
  ...
  Promise.all(createJsonPromises).then((files) => {
    ...
    fs.promises
      ...
      for (let i=0; i<unzippedFiles.length; i++) {
        cleanupPromises.push(fs.promises.unlink(path.join(config.LOGS, unzippedFiles[i])))
      }
