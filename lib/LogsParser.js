...
const { promiseStringify, promiseParse } = require("./helpers/JsonPromises");
...
function jsonFromLogfilePromise(filepath) {
  return new Promise((resolve, reject) => {
    ...
    const cachedLogJsonFilepath = path.join(CACHED_JSON_LOG_DIR, path.basename(filepath).replace(/\.log$/g, ".json"));
    const createdJSON = [];
    const createdDate = getDateFromFilename(filepath);

    if (fs.existsSync(cachedLogJsonFilepath)) {
      // Check if we created a cached version already, and if it's more recent
      if (
        fs.statSync(cachedLogJsonFilepath).mtime >= fs.statSync(filepath).mtime ||
        unzippedFiles.indexOf(path.basename(filepath)) > -1
      ) {
        // Our cached version is at least as recent as the version we need to parse
        // Or the cached version matches a version which we extracted from an archive

        // Had to make this synchronous
        try {
          const filedata = fs.readFileSync(cachedLogJsonFilepath);
          const logjson = JSON.parse(filedata);
          rawlogJSON.push(...logjson);
          logger.verbose(`Able to use cached JSON version of ${path.basename(filepath)}`, { domain: DOMAIN });
          return resolve(filepath);
        } catch (err) {
          logger.verbose(`Unable to use cached JSON version of ${path.basename(filepath)}`, {
            domain: DOMAIN,
          });
          logger.debug(err, { domain: DOMAIN });
        }
      }
    }
    ...
    try {
      ...
      rl.on("close", () => {
        logger.verbose(`Completed parsing ${filepath}`, { domain: DOMAIN });

        promiseStringify(createdJSON)
          .then((stringified) => {
            return fs.promises.writeFile(cachedLogJsonFilepath, stringified);
          })
...
function parseLogFiles() {
  ...
  return Promise.all(createJsonPromises)
    ...
    .then((res) => {
      const time = `${process.hrtime(TIMERS.presort)[0]}s, ${process.hrtime(TIMERS.presort)[1] / 1000000}ms`;
