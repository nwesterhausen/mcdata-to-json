...
function mclogToJson(logfilename) {
  ...
  if (fs.existsSync(path.join(LOG_JSON_CACHE_DIR, `${logfilename.split(".")[0]}.json`))) {
    if (
      fs.statSync(LOG_JSON_CACHE_DIR, `${logfilename.split(".")[0]}.json`).mtime >
      ...
      if (FILE_EXTENSION === ".log") {
        ...
        let logjs = fs.readJSONSync(path.join(LOG_JSON_CACHE_DIR, `${logfilename.split(".")[0]}.json`));
  ...
  return new Promise((resolve, reject) => {
    ...
    const linereader = readline.createInterface({
      input: fs.createReadStream(path.join(Config.LOGS_DIR, logfilename)),
    });
    linereader.on("line", (input) => {
      LogfileTools.appendLogActionTo(createdJSON, LogfileTools.parseLogLine(createdDate, input));
    });
    linereader.on("close", () => {
      if (createdJSON.length === 0) {
        ...
        fs.writeJSON(path.join(LOG_JSON_CACHE_DIR, `${logfilename.replace(/.log/, ".json")}`), createdJSON, {
          spaces: 2,
        })
          .then((val) => {
...
function unzipLogFile(gzipLogfile) {
  ...
  fs.writeFileSync(path.join(Config.LOGS_DIR, gzipLogfile.replace(/.gz/, "")), unzippedData);
...
function buildCombinedLogfiles() {
  ...
  cleanedJSON = rawlogJSON.filter((obj) => {
    return (
      [
        Constants.TYPE_KEEPENTITY,
        Constants.TYPE_OVERLOADED,
        Constants.TYPE_MOVEDQUICKLY,
        Constants.TYPE_PREPARESPAWN,
        Constants.TYPE_ARGUMENTABIGUITY,
      ].indexOf(obj.type) === -1
    );
  });
  ...
  Log.debug(
    `Wrote 'cleaned' JSON file to ${path.join(LOG_JSON_CACHE_DIR, "filtered_logs.json")} (${
      cleanedJSON.length
    } records)`,
    DOMAIN
  );
  ...
  Log.debug(
    `Wrote 'chat' JSON file to ${path.join(LOG_JSON_CACHE_DIR, "chat.json")} (${chatJSON.length} records)`,
    DOMAIN
  );
  ...
  Log.debug(
    `Wrote 'command' JSON file to ${path.join(LOG_JSON_CACHE_DIR, "command.json")} (${commandJSON.length} records)`,
    DOMAIN
  );
  ...
  Log.debug(
    `Wrote 'command' JSON file to ${path.join(LOG_JSON_CACHE_DIR, "deaths.json")} (${deathJSON.length} records)`,
    DOMAIN
  );
}
