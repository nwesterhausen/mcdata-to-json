...
import { MCLevelData, PlayerData } from './Interfaces';

const path = require("path");
const fs = require("fs");

const starttime = process.hrtime();
const hrtimefmt = function (timeref: [number, number]) {
...
if (ServerDataExtractor.checkForData()) {
  fs.promises.writeFile(path.join(PATHS.OUTPUT_DIR, "uuids.json"), JSON.stringify(Config.PLAYERS)).catch((e: Error) => {
    throw e;
  });

  ServerDataExtractor.convertLevelDat()
    .then((level: MCLevelData) => {
