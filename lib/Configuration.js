...
const helpMessage = `mcdata-to-json ${version.version}
...
const PROPERTIES_FILE = path.join(MC, "server.properties");
const LOGS = path.join(MC, "logs");
const WORLD = path.join(MC, "world");
const STATS = path.join(WORLD, "stats");
const ADVANCEMENTS = path.join(WORLD, "advancements");
...
try {
  ...
  if (err.code === "ENOENT") {
    const testedPath = path.basename(err.path);
