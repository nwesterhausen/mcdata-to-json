...
if (parsedOpts["use-env"]) {
  log.debug(DOMAIN, "use-env flag set, loading config from environment.");
  ...
  log.debug(DOMAIN, `WORK_DIR: ${process.env.WORK_DIR}`);
...
log.debug(DOMAIN, `Checking Minecraft dir: ${parsedOpts.minecraft}`);
...
log.info(DOMAIN, "Validated minecraft directory.");
