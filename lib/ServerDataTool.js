...
function checkForData() {
  ...
  if (fs.existsSync(path.join(tempRoot, "data"))) {
    if (fs.existsSync(path.join(tempRoot, "data", "minecraft"))) {
      advancementsExported = fs.existsSync(
        path.join(tempRoot, "data", "minecraft", "advancements")
      );
      loottablesExported = fs.existsSync(
        path.join(tempRoot, "data", "minecraft", "loot_tables")
      );
      recipesExported = fs.existsSync(
        path.join(tempRoot, "data", "minecraft", "recipes")
      );
      tagsExported = fs.existsSync(
        path.join(tempRoot, "data", "minecraft", "tags")
      );
    }
    if (fs.existsSync(path.join(tempRoot, "data", "reports"))) {
      blocklistExported = fs.existsSync(
        path.join(tempRoot, "data", "reports", "blocks.json")
      );
      blocklistExported = fs.existsSync(
        path.join(tempRoot, "data", "reports", "commands.json")
      );
      blocklistExported = fs.existsSync(
        path.join(tempRoot, "data", "reports", "registries.json")
      );
    }
  }
...
function exportMinecraftDataPromise() {
  return new Promise((resolve, reject) => {
    ...
    if (tempRoot === "unset" || serverjarPath === "unset") {
      log.error(
        DOMAIN,
        "Tried to run data generation without setting serverjar and/or output folder."
      );
...
function ensureMinecraftAdvancements() {
  ...
  if (!advancementsExported) {
    ...
    log.info(
      DOMAIN,
      `Using cached minecraft advancements in ${path.join(
        tempRoot,
        "data",
        "minecraft",
        "advancements"
